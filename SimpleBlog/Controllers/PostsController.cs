...
namespace SimpleBlog.Controllers
{
    public class PostsController : Controller    
    {
        ...
        public ActionResult Index(int page = 1)
        {
            ...
            var baseQuery = Database.Session.Query<Post>().Where(t => t.DeletedAt == null).OrderByDescending(t => t.CraetedAt);

            var totalPostCount = baseQuery.Count();
            var postIds = baseQuery.Skip((page - 1) * PostsPerPage).Take(PostsPerPage).Select(t => t.Id).ToArray();
            var posts = baseQuery.Where(t => postIds.Contains(t.Id)).Fetch(t => t.Tags).Fetch(t => t.User).ToList();
        ...
        public ActionResult Tag(string idAndSlug, int page = 1)
        {
            ...
            var postIds = tag.Posts
                .OrderByDescending(g => g.CraetedAt)
            ...
            var posts = Database.Session.Query<Post>()
                .OrderByDescending(b => b.CraetedAt)
